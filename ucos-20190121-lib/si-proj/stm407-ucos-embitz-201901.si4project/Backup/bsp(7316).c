...
void  BSP_Init (void)
{
//    BSP_IntInit();      //��ÿ���ж�Դ�����жϴ������������Ҫ����

    //��Ϊʱ�ӳ�ʼ���Ѿ���SystemInit�������ˣ������޸�

    BSP_LED_Init();                                             /* Init LEDs.                                           */
    BSP_UartOpen(COM1,  115200, 8, 1, 0);       //���ڳ�ʼ��
...
INT32U  BSP_CPU_ClkFreq (void)
{
	...
	SystemCoreClockUpdate();        //embitz 自带了这个函数，20190202
	return SystemCoreClock;			//全局变量保存了cpu的时钟，单位Hz
#endif
}
...
#else
    cnts  = cpu_clk_freq / (INT32U)OS_TICKS_PER_SEC;        /* Determine nbr SysTick increments.                    */
#endif
...
void  BSP_LED_On (INT8U  led)
{
     if(led< 4)
    {
        GPIO_SetBits(GPIOE, led_arr[led]);
    }
}
...
void  BSP_LED_Off (INT8U led)
{
     if(led< 4)
    {
        GPIO_ResetBits(GPIOE, led_arr[led]);
    }
}
...
void  BSP_LED_Toggle (INT8U  led)
{
    ...
    if(led< 4)
    {
        ...
        pins ^= led_arr[led];
        GPIO_SetBits(  GPIOE,   pins  & (led_arr[led]));
        GPIO_ResetBits(GPIOE, (~pins) & (led_arr[led]));
    }
}
...
//    BSP_REG_DEM_CR     |= (INT32U)BSP_BIT_DEM_CR_TRCENA;    /* Enable Cortex-M4's DWT CYCCNT reg.                   */
//    BSP_REG_DWT_CYCCNT  = (INT32U)0u;
//    BSP_REG_DWT_CR     |= (INT32U)BSP_BIT_DWT_CR_CYCCNTENA;
